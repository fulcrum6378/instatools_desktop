...
object RelayPrefetchedStreamCache {
    ...
    fun crawl(
        ...
        var read = html
        var json: String
        var gson: Map<String, Array<Array<Any>>>
        var tuple: Array<Any>
        ...
        while (read.contains(HTML_TAG_SCRIPT_JSON)) {
            ...
            if (json.contains("RelayPrefetchedStreamCache") && predicate(json)) {
                gson = (GsonBuilder().setLenient().create()
                    .fromJson(json, gsonType) as Map<String, Array<Array<Any>>>)
                gson = (gson["require"]!![0][3] as Array<Map<String, Any?>>)[0]["__bbox"]!!
                        as Map<String, Array<Array<Any>>>
                tuple = gson["require"]!![0][3] as Array<Any>
